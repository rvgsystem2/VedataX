<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3 bg-white shadow-md px-6 py-4 rounded-lg">
            <div>
                <h2 class="font-bold text-2xl text-gray-800">{{ __('Add Home Page Media') }}</h2>
                <p class="text-sm text-gray-500">Upload hero, locations, about image & final category background.</p>
            </div>
    ...
    <div class="mt-10 md:px-8 lg:px-16 pb-10">
        ...
        <form action="{{ route('home.media.store') }}" method="POST" enctype="multipart/form-data"
            ...
            {{-- create me media null hoga --}}
            @include('admin.home_page_media.partials.form', ['media' => null])
